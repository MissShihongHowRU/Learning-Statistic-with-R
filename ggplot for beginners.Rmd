---
title: "ggplot: a quick tutorial"
resource: stats-club, Applied Statistics NYU Steinhardt 
output: pdf_document
---

The Original Source of This Toturial: 
https://github.com/priism-center/stats-club
...
p1 <- p1 + geom_smooth(method = "lm", se = F, linetype = "dashed")  # "solid"
...
```{r, echo = F}  
p2 <- ggplot(mtcars, aes(factor(cyl), mpg))  ##always factor your variable
p2 <- p2 + geom_boxplot()
p2 <- p2 + geom_point(aes(col = factor(vs)), size = 2, alpha = 0.5) ## (col = ) is in the aes()
p2 <- p2 + theme_bw()  
p2
```
...
We can use different aesthetics to represent more dimensions (but don't overkill!).
(Although putting all aesthetics in one **aes()** yields a same image)
```{r}
ggplot(mtcars, aes(x = wt, y = mpg)) + geom_point(aes(col = factor(cyl), fill = factor(cyl), shape = factor(vs), size = 2*(hp)/mean(hp)))
...
```{r}
b_df <- data.frame(x = rbeta(1000, 5, 5))
b <- ggplot(b_df, aes(x))
#b <- b + geom_histogram(aes(y = stat(density)), binwidth = 0.08, col = "black", fill = "#ffffff")
b <- b + geom_density(fill ='#ff6666', alpha = 0.5) + theme_bw()
b 
...
# Option 3
p5 + stat_density2d(aes(fill = stat(density)), geom = "tile", contour = F)
p5
```
...
```{r}
n_df <- data.frame(X = rnorm(10000, 0, 1), Y = rnorm(10000, 0, 1))

pc3 <- ggplot(n_df, aes(X, Y)) + geom_point()
pc3 <- pc3 + geom_density2d()

pc3 <- pc3 + stat_density2d(aes(fill = stat(density)), geom = "raster", contour = F)
pc3 + scale_fill_distiller(palette = "Spectral", direction = -1)
...
```{r}
# tip: put factor()inside aes()
pc4 <- ggplot(mtcars, aes(x = factor(cyl), y = mpg)) + geom_boxplot(color = "lightgreen", alpha = 0.1) 
pc4 <- pc4 + geom_point(size = 2, alpha = 0.5, aes(col = factor(vs)))  ## 若不在aes()内则需重新指定x,y
pc4 <- pc4 + scale_color_manual(values = c("0" = "blue", "1" = "red"), labels = c("V", "Straight"), 
                                guide = guide_legend(title = str_wrap("Engine Configuration", width = 12)))
pc4 <- pc4 + xlab(label = "Cylinder") + ylab(label = "Miles Per Gallon (MPG)")
pc4 + theme_bw()
...
p10 <- p10 + geom_bar(aes(y = ..count.., fill = factor(vs)), position = "dodge", alpha = 0.5) #default "stack", jitter 
...
```{r}
pgap <- ggplot(data = gapminder, aes(gdpPercap, lifeExp))
pgap <- pgap + geom_point(size = pop, show.legend = F, aphla = 0.6) +
...
```{r}
# piping, pip the result of the last command as the first arguement of the next command
p12 <- gapminder %>% filter(year == 1977) %>% ggplot(aes(gdpPercap, lifeExp)) + 
  geom_point(aes(fill = factor(continent), size = pop), shape = 21, show.legend = F, alpha = 0.6) + 
  scale_size_area(max_size = 30) + 
  scale_x_log10(limits = c(150, 120000)) + ylim(c(20, 90)) + 
  scale_fill_manual(values = continent_colors, aesthetics = c("colour", "fill")) +
  theme_bw()
p12 <- p12 + geom_text(data = gapminder %>% filter(year == 1977, pop >= 1e8), aes(label = country), size = 3, check_overlap = T, show.legend = F)
p12
```
